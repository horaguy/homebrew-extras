...
# Usage: bump-pr.sh <package> [version]

BREW_TAP_NAME="$(brew tap)"

function main(){
  FULL_PKG_NAME="$BREW_TAP_NAME/$1"
  VERSION=$2
  IS_CASK=false
  if [ "$3" = "--cask" ]; then
    IS_CASK=true
  ...
  if [ -z "$VERSION" ]; then
    VERSION=$(brew livecheck --json "$FULL_PKG_NAME" | jq -r '.[0].version.latest')
    if [ -z "$VERSION" ]; then
      echo "Failed to get latest version"
      exit 1
    fi
  fi
  create_merge_bump_pr "$FULL_PKG_NAME" "$VERSION" "$IS_CASK"
}
...
function create_merge_bump_pr() {
  ...
  if [ "$IS_CASK" = true ]; then
    brew bump-cask-pr "$FULL_PKG_NAME" --no-browse --no-fork --version "$VERSION" > tmp.log 2>&1; ret=$?; cat tmp.log
  else
    brew bump-formula-pr "$FULL_PKG_NAME" --no-browse --no-fork --version "$VERSION" > tmp.log 2>&1; ret=$?; cat tmp.log
  fi
